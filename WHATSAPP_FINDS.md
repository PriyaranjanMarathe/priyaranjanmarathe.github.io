...
Save commands accept leading/trailing whitespace, tabs and nonbreaking spaces between tags, Windows/Unix line endings, blank separator lines, case-insensitive labels, indentation and spaces before field colons. Titles are trimmed. Duplicate fields, empty titles, overlong fields and ambiguous command text remain rejected. The importer emits message-free Actions warnings for malformed commands, missing reply links and replies outside the selection window. These warnings are in Actions logs; no WhatsApp acknowledgement is sent.

Common intended save syntax is recovered: `save:`, bold WhatsApp labels, comma/semicolon-separated hashtags, final punctuation, `Titel:` and `Notes:`. A save without a reply link can select exactly one preceding attachment within ten minutes; the importer looks back across polling boundaries. Multiple candidates, no candidates or an explicit missing reply target are not guessed. An explicit `save` is still required, and filenames/content are never inferred from unrelated messages.

## iNaturalist observation previews

Share an `https://www.inaturalist.org/observations/123` link and select it with the usual save reply. The importer fetches observation metadata from the public iNaturalist API, then embeds one eligible photo per observation (up to three observation links per post), with species name, photographer attribution and a link to the photo's licensing details. Custom titles and notes still work normally. Public Creative Commons/CC0 photos are supported for this noncommercial personal site; all-rights-reserved, hidden or unrecognized-host photos remain links only.

Photo bytes stay on iNaturalist's infrastructure: preview metadata is separate from uploaded attachments and never enters Vercel Blob or R2. External availability depends on the source. An API outage does not block the post: the original link publishes, and later runs retry enrichment. Successful responses are cached in `link_previews`; remove that field from an affected record to refresh its metadata. No observation coordinates or other location metadata are copied. Only selected post text/notes are inspected, not all incoming messages. Existing selected observation links are enriched on later publishing runs.
